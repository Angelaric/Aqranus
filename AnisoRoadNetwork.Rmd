...
plot(Locations$geometry, cex=2, pch = 19, col="blue", add=TRUE) # The locations
...
But first, let load the converted into points Wendel's road sistem as a point shapefile with points in every 500 m so we can use it as features set on the road.
The file is created in QGIS with Convert Lines to Polygons. In field "Lines" the vectorised Wendel's roads file is loaded. Then check the field "Insert Additional Points". Insert "Distance = 500 m". Check "Add Point Order".In this way we have converted the shapefile from line to points with point in every 500 m. Then leave only the fields "FID", "Shape", "Type", "Shape Lenght" and "PT ID". We will need them. 

```{r xxi}

# Plot it

plot(WendelRoadsPoints$geometry, cex=0.5, pch = 19, col="black")
...
plot(network, col="black")
plot(sf::st_geometry(access2), col = "red", pch = 20, cex=2, add = TRUE);
plot(sf::st_geometry(access1), col = "green", pch = 20, cex=1, add = TRUE);
plot(sf::st_geometry(access3), col = "yellow", pch = 20, cex=0.5, add = TRUE);
