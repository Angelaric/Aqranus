...
plot(Territory2); #This is the DEM we need. It includes river Thundza as a barrier and the fords for crossing it. But this file is too big for GitHub and I will ignore it. I don't know any other way how to share it. A simple DEM for the region is enough too. It is not necessary to have the River as a barrier.
...
Trinabos_Beroe <- movecomp(dtm=Ter2Agg10, origin = as(Origo_Destino[2,], "Spatial"), destin=as(Origo_Destino[1,], "Spatial"), choice=c("t"), barrier=Ridges, field = 3, move=8, export = FALSE)
...
Strinabos_Beroe <- movecomp(dtm=Ter2Agg10, origin = as(Origo_Destino[3,], "Spatial"), destin=as(Origo_Destino[1,], "Spatial"), choice=c("t"), barrier=Ridges, field = 3, move=8, export = FALSE)
...
plot(Ter2Agg10); plot(Origo_Destino$geometry, col = "red", pch = 19, add=TRUE); plot(Trinabos_Beroe$LCPs, col = "blue", add=TRUE); plot(Strinabos_Beroe$LCPs, col = "black", add=TRUE)
