...
So, we use sfnetwors() and follow the instructions of Lucas van der Meer from here:
...
It is generally accepted that a person walks 1 km in 12 minutes or 5 km in 1 hour. Accordingly, if Lufisa is 149.2 km away from Kran, we need to input into the code:

149.2 km / 5 km per hour = 29.8 hours,
which in minutes should be 1,790.8 min (149.2 km × 12 min = 1,790.8 min).

The same applies to the other locations:

Istibuni = 223.9 km × 12 min = 2,686.8 min
...
# Istibuni = 223.9 km × 12 min = 2.686 min (we round)
...
access3 = WendelRoadsPoints[which(times < units::set_units(2686, "min")),]
...
# From Lufisa to Kran is 149.2  km / 5 km per hour = 29.8 hours, which in minutes should be 1,790.4 min (149.2 km × 12 min = 1,790.4 min).
...
access1 = WendelRoadsPoints[which(times < units::set_units(1790, "min")),] # We round the minutes to 1738
...
access32 = WendelRoadsPoints[which(times < units::set_units(2574, "min")),]
...
plot(network, col="black")
plot(sf::st_geometry(access32), col = "yellow", pch = 20, cex=1, add = TRUE)
plot(Locations[3,], pch = 19, col="blue", add=TRUE, cex = 1.5)
...
access12 = WendelRoadsPoints[which(times < units::set_units(1716, "min")),]
...
# Plot the results
plot(network, col="black")
plot(sf::st_geometry(access12), col = "green", pch = 20, cex=1, add = TRUE)
plot(Locations[1,], pch = 19, col="blue", add=TRUE, cex = 1.5)
...
plot(network, col="black")
plot(sf::st_geometry(access2), col = "red", pch = 20, cex=2, add = TRUE, alpha = 0.5);
plot(sf::st_geometry(access12), col = "yellow", pch = 20, cex=1, add = TRUE, alpha = 0.5);
plot(sf::st_geometry(access32), col = "green", pch = 20, cex=0.5, add = TRUE, alpha = 0.5);
plot(Locations, pch = 19, col="blue", add=TRUE, cex = 1.5)
...
mapview(WendelRoads, lwd = 0.5) + 
mapview(access12, col.regions = "yellow", layer.name = "Lufisa") +
mapview(access2, col.regions = "red", layer.name = "Farui") +
mapview(access32, col.regions = "green", layer.name = "Istibuni")
...
# Connect each point to its nearest neighbor (excluding itself)
lines_nn12 <- st_connect(x=access12, y=access12, ids = NULL, k = 2)

# Filter out self-connections (if any)
lines_nn12 <- lines_nn12[unclass(st_length(lines_nn12)) > 0 ] %>% 
  st_sf()

# repeat for Lufisa
# Connect each point to its nearest neighbor (excluding itself)
lines_nn32 <- st_connect(x=access32, y=access32, ids = NULL, k = 2)

# Filter out self-connections (if any)
lines_nn32 <- lines_nn32[unclass(st_length(lines_nn32)) > 0 ] %>% 
  st_sf()
...
```{r xliii}
mapview(WendelRoads, lwd = 0.5) + 
mapview(lines_nn12, color = "yellow", layer.name = "Lufisa") +
mapview(lines_nn2, color = "red", alpha = 0.7, layer.name = "Farui") +
mapview(lines_nn32, color = "green", alpha = 0.7, layer.name = "Istibuni")

```


```{r xliv}
lines_nn12$source <- "Lufisa"
lines_nn12$lwidth <- 3
lines_nn2$source <- "Farui"
lines_nn2$lwidth <- 1
lines_nn32$source <- "Istibuni"
lines_nn32$lwidth <- 2
lines_all2 <- bind_rows(lines_nn32, lines_nn12, lines_nn2)
...
LinesLufisa1716 <- here("results", "LufisaNetwork1716min.shp")
# Export the results
st_write(lines_nn12, LinesLufisa1716, delete_layer = TRUE)

# Define the output file path for the Istibuni network
LinesIstibuni2574 <- here("results", "IstibuniNetwork2574min.shp")
# Export the results
st_write(lines_nn32, LinesIstibuni2574, delete_layer = TRUE)
