...
In this file we demonstrate the Anisotropic Road Network Analysis on the base of sfnetwork() function. 
The calculated distances are the same as those in AqranusIsochrones.Rmd

So, we use sfnetwors() and follow the instructions of Van der Meert et al. (2024) from here:
https://luukvdmeer.github.io/UseR2024/notebook.html#compute-accessibility-to-restaurants

and adapt it for our own needs
...
# plot(Territory2) # the 30 m resolution DEM
...
# Now we can set a speed of each road segment, based on its gradient. We use a default speed of 4.9 km/h. An uphill gradient decreases this speed, and a downhill gradient increases this speed, all until a given minimum and maximum speed of respectively 1.9 km/h and 5.9 km/h.

estimate_pedestrian_speed <- function(gradient, default = 4.9, max = 5.9, min = 1.9) {
  ifelse(
    gradient < 0,
...
# The function is pedestrian modification of the original function for cycling, made by Van der Meer et al (2024).
# Modification principle:
# Base speed: 4.9 km/h
# Maximum speed: 5.9 km/h (on a gentle descent)
# Minimum speed: 1.9 km/h (on a steep climb)
...
plot(dplyr::select(WendelRoads, time))
...
access2 = WendelRoadsPoints[which(times < units::set_units(756, "min")),]
...
# From Istibuni to Aqranus is 6 marahil x 7 h = 28 h x 60 min = 2520 min
...
access3 = WendelRoadsPoints[which(times < units::set_units(2520, "min")),]
...
# From Lufisa to Aqranus is 4 marahil x 7 h = 28 h x 60 min = 1680 min
...
access1 = WendelRoadsPoints[which(times < units::set_units(1680, "min")),] # We round the minutes to 1738
...
And export the line network results if it is necessary

```{r xxxv}

# Define the output file path for Lufisa network

LinesLufisa <- here("results", "LufisaNetwork1680min.shp")
# Export the results
st_write(lines_nn1, LinesLufisa, delete_layer = TRUE)

# Define the output file path for Farui network
LinesFarui <- here("results", "FaruiNetwork756min.shp")
# Export the results
st_write(lines_nn2, LinesFarui, delete_layer = TRUE)

# Define the output file path for the Istibuni network
LinesIstibuni <- here("results", "IstibuniNetwork2520min.shp")
# Export the results
st_write(lines_nn3, LinesIstibuni, delete_layer = TRUE)
...
Van der Meert, Lucas, Lorena Abad, Andrea Gilardi, and Robin Lovelace. 2024. ‘Sfnetworks: Tidy Geospatial Networks’. R. CRAN (The Comprehensive R Archive Network). https://CRAN.R-project.org/package=sfnetworks.

Wendel, Michael. 2005. Die Verkehrsanbindung in frühbyzantinischer Zeit 4.-8. Jh. n. Chr. - Karasura III. Vol. III. ZAKS Schriften 6. Langenweißbach: Beier & Beran.
